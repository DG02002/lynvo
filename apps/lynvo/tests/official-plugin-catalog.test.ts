  ...
  it("uses the request origin for development icons published on loopback", () => {
    expect(
      resolveOfficialPluginIconUrl(
        "http://localhost:5173/official-extractor-assets/icons/sources/onedrive-index.webp",
        "http://192.168.1.3:5173/settings"
      )
    ).toBe(
      "http://192.168.1.3:5173/official-extractor-assets/icons/sources/onedrive-index.webp"
    )
    expect(
      resolveOfficialPluginIconUrl(
