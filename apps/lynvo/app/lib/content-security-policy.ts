export const createContentSecurityPolicy = (
  ...
  return `default-src 'self'; script-src ${scriptPolicy}; frame-src 'self' https://challenges.cloudflare.com; style-src 'self' 'unsafe-inline'; font-src 'self' data:; img-src 'self' data: https: http://localhost:* http://127.0.0.1:*${developmentImageSource}; connect-src 'self' https://challenges.cloudflare.com https://*.convex.cloud wss://*.convex.cloud${developmentSources};`
}
