...
const draftToRecentLinkViewItem = (
...
export function useDraftRecentLinks(recents: RecentLinkViewItem[]) {
  const [drafts, setDrafts] = useState<RecentLinkViewItem[]>([])

  const refreshDrafts = useCallback(() => {
    setDrafts(getExpiringDrafts().map(draftToRecentLinkViewItem))
  }, [])
  ...
  return useMemo(() => {
    ...
    const filteredRecents = recents.filter(
      (recent) => !draftUrls.has(recent.url)
    )
    return [...drafts, ...filteredRecents]
  }, [drafts, recents])
}
