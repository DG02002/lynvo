...
A Plugin owns the matching, upstream requests, and Media Node mapping for one Source. Read [what a Plugin Server does](/docs/plugin-server/what-is-a-plugin-server), then define its metadata in the [manifest](/docs/plugin-server/manifest) and choose the right [Media Node types](/docs/plugin-server/media-nodes).

The generated project includes one deterministic example Plugin. Replace it with
your Source implementation before deployment.
...
A Plugin owns Source credentials, upstream requests, response parsing, Media
Node creation, and Source-specific errors. The shared runtime owns bearer
authentication, protocol validation, usage enforcement, and response
serialization.

For multiple Plugins, keep one catalog for manifest metadata and request
dispatch. Do not add Source-specific conditions to the Hono route layer.
