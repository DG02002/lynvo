...
Generate a standalone Cloudflare Worker project to start a Custom Plugin Server. Complete the [development prerequisites](/docs/plugin-server/prerequisites), then replace its example Plugin and follow the [agent-assisted path](/docs/plugin-server/agent-prompt) or the [protocol guide](/docs/plugin-server/protocol-overview).
...
The `pnpm build` script runs a Wrangler dry run. Use `pnpm deploy` only after
the contract checks pass and the production secret is configured.

The example usage response is a finite test fixture. Replace it with durable accounting before production deployment.

The generated project has no workspace or local-link dependency, so you can
copy it to another repository without cloning Lynvo.
