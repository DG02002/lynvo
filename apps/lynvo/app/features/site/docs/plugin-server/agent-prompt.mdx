...
description: Use a coding agent to replace the starter Plugin without rebuilding the Worker runtime.
...
Use this guide after generating the starter project when you want an agent to implement a Source-specific Plugin. First [prepare your environment](/docs/plugin-server/prerequisites) and [generate the project](/docs/plugin-server/create-plugin-server); then use [protocol testing](/docs/plugin-server/testing) before deployment.

Generate the project with `pnpm create lynvo-plugin-server@latest`. Run the
prompt below from the generated project directory. It asks the agent to extend
the existing Worker instead of rebuilding its route and validation code.
...
The agent should edit the Plugin implementation, manifest metadata, and tests.
Keep the shared runtime and route wiring unchanged unless the protocol requires
