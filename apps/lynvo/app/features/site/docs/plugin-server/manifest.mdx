...
The manifest declares a Custom Plugin Server's identity, protocol features, and Plugins. Read the [protocol overview](/docs/plugin-server/protocol-overview), then match each Plugin entry to the [Source implementation](/docs/plugin-server/plugins).
...
| `features`        | Yes      | Declares password, unresolved item, and Basic Auth support       |
