...
const TermsOfUseServiceContent = () => (
  <>
    <p>
      Lynvo saves URLs and related metadata, resolves supported pages into
      playable links, synchronizes records across your sessions, and opens links
      in Just (Video) Player, VLC for Android, MPV, or MX Player on Android
      phones, Android tablets, and Android TV. Lynvo does not play video. It is
      a link-management and player-handoff service, not a video catalog, media
      seller, or subscription to third-party content.
    </p>
    <p>
...
const TermsOfUseEligibilityContent = () => (
  <>
    <p>
      You must be at least 13 years old and meet the minimum age for online
      consent where you live. If local law requires permission from a parent or
      guardian, you must have that permission before using Lynvo.
    </p>
    <p>
      If you use Lynvo for an organization, you confirm that you have authority
...
const TermsOfUseRetentionContent = () => (
  <>
    <p>
...
const TermsOfUseLimitsContent = () => (
  <>
    <p>
      ...
      1,000 saved links. These limits cover stored records and metadata, not the
      size of a linked video file.
    </p>
    <p>
      Lynvo Plugin Server extraction requests, including Direct Media, share an
      allowance of 30 requests per day and 200 requests per month. Custom Plugin
      Servers report and enforce their own finite limits.
    </p>
    <p>
      Lynvo may reject a record, require you to remove data, or change limits to
      protect reliability and prevent abuse. Lynvo will update these terms or
      the relevant interface if the standard limits change.
    </p>
  </>
)
...
const TermsOfUsePluginsContent = () => (
  <>
    ...
    <p>
      Sources can change or remove files, require authorization, block requests,
      or return unsafe material. Just (Video) Player, VLC for Android, MPV, and
      MX Player may handle links differently. Lynvo doesn&apos;t endorse or
      guarantee third-party services, files, Plugins, Plugin Servers, or
      players.
    </p>
  </>
)
...
const TermsOfUseTerminationContent = () => (
  <>
    ...
    <p>
      Lynvo may limit, suspend, or terminate access if you violate these terms,
      create security or legal risk, abuse Lynvo, or require resources that
      threaten Lynvo reliability. Lynvo may also remove links or configuration
      when law requires it.
    </p>
  </>
)

const TermsOfUseAvailabilityContent = () => (
  <>
    <p>
      Lynvo is an evolving service. Features, supported Sources, the four
      supported Android players, limits, and availability may change. Lynvo may
...
const TermsOfUseDisclaimerContent = () => (
  <>
    <p>
...
const TermsOfUseLiabilityContent = () => (
  <p>
    To the fullest extent law permits, Lynvo and its contributors won&apos;t be
    liable for indirect, incidental, special, consequential, exemplary, or
    punitive damages. This includes loss of data, links, access, profits,
    goodwill, or use arising from Lynvo or a third-party service.
  </p>
)

const TermsOfUseLiabilityDetails = () => (
  <p>
    Where law doesn&apos;t allow a complete exclusion, Lynvo&apos;s total
    liability for claims related to Lynvo won&apos;t exceed the amount you paid
    to Lynvo during the 12 months before the event giving rise to the claim.
    Some jurisdictions don&apos;t allow certain exclusions, so parts of this
    section may not apply to you.
  </p>
)

const TermsOfUseCopyrightContent = () => (
  <p>
    For a rights report, use <SupportChannelLinks />. Include the URL, the right
    you own or represent, your contact details, and enough information to
    evaluate the report. Lynvo may remove access to reported material or suspend
    repeat violators when appropriate.
  </p>
)

const TermsOfUseChangesContent = () => (
  <p>
    Lynvo may update these terms when Lynvo or legal requirements change. The
    updated date identifies the current version. Lynvo will provide additional
    notice when law requires it. Your continued use after an update means you
    accept the revised terms.
  </p>
)

const TermsOfUseGeneralContent = () => (
  <>
    <p>
      ...
      provisions continue to apply. A failure to enforce a provision isn&apos;t
      a waiver of it.
    </p>
    <p>
      You may not transfer these terms without Lynvo&apos;s permission. Lynvo
      may transfer them as part of a reorganization, acquisition, or transfer of
      Lynvo. For questions about these terms, use <SupportChannelLinks />.
    </p>
  </>
)
